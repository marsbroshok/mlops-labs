...
By default, all regional resources are provisioned in the `us-central1` region and all zonal  are provisioned in the `us-central1-a` zone. Although not recommened, you can use another region or zone by providing their names as the second and thirg parameters to the `install.sh` script.
